...
public class ShowLevels : MonoBehaviour
{
    ...
    private GameObject[] labell = new GameObject[10];
    ...
    void ShowLevelsNumber(int page)
    {
        int compteur = 0;
        for (int i = 0; compteur < minXAnchor.Length; i++, compteur++ )
        {
            if (i + 1 + (page * 10) <= GameManager.NbLevels)
            {
                labell[i%10] = Instantiate(buttonLevel, transform);

                RectTransform newDoor = labell[i%10].GetComponent<RectTransform>();
                ...
                TextMeshProUGUI levelLabell = labell[i%10].GetComponentInChildren<TextMeshProUGUI>();
                levelLabell.text = (i + 1 + (page * 10)).ToString();
            }
